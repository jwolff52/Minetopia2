package io.github.jwolff52.minetopia2.inventory;
...
public class ContainerAlchemyChest extends ContainerM2{
    ...
    public ContainerAlchemyChest(InventoryPlayer inventoryPlayer, TileEntityAlchemyChest tileEntityAlchemyChest) {
        ...
        switch(this.tileEntityAlchemyChest.getState()) {
            case 1:
        ...
        for(int chestRowIndex = 0; chestRowIndex < chestInventoryRows; chestRowIndex++) {
            for(int chestColumnIndex = 0; chestColumnIndex < chestInventoryColumns; chestColumnIndex++) {
                switch(this.tileEntityAlchemyChest.getState()) {
                    case 2:
                        this.addSlotToContainer(new Slot(tileEntityAlchemyChest, chestColumnIndex + chestRowIndex * chestInventoryColumns, 8 + chestColumnIndex * 18, 8 + chestRowIndex * 18));
                        break;
                    default:
                        this.addSlotToContainer(new Slot(tileEntityAlchemyChest, chestColumnIndex + chestRowIndex * chestInventoryColumns, 8 + chestColumnIndex * 18, 18 + chestRowIndex * 18));
                }
            }
        }
    ...
    public void onContainerClosed(EntityPlayer entityPlayer) {
        ...
        tileEntityAlchemyChest.closeInventory();
    }

    @Override
    public boolean canInteractWith(EntityPlayer entityPlayer) {
        return this.tileEntityAlchemyChest.isUseableByPlayer(entityPlayer);
    }
